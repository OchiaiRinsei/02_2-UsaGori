...
void title(struct DATA* d){
    ...
    init(d);
    rectMode(CENTER);
    image(d->titleImg, width/2, height/2);
    if (d->state == d->TITLE && isTrigger(KEY_SPACE)) {
        d->state = d->PLAY;
    }
}
void init(struct DATA* d){
    ...
    d->rabit.hp = 50;
    //弾発射オフセット位置
    d->rabit.bulletOfsY = -70.0f;//pyにこの値を加算した値を発射位置とする
    d->rabit.hpGaugeOfsY = -60.0f;
    //当たり判定用、半分の幅と高さ
    d->rabit.halfW = 37.0f;
    d->rabit.halfH = 55.5f;
    ...
    d->gori.hp = 100;
    //弾発射オフセット位置
    d->gori.bulletOfsY = 70.0f;//pyにこの値を加算した値を発射位置とする
    d->gori.hpGaugeOfsY = -60.0f;
    //当たり判定用、半分の幅と高さ
    d->gori.halfW = 55.0f;
    d->gori.halfH = 62.0f;

    //人参の初期化
    d->ninjin.img = d->ninjin.img;
    d->ninjin.px = 0.0f;
    d->ninjin.py = 0.0f;
    d->ninjin.vx = 0.0f;
    d->ninjin.vy = -6.0f;
    ...
    d->ninjin.hp = 0;
    //当たり判定用、半分の幅と高さ
    d->ninjin.halfW = 15.0f;
    d->ninjin.halfH = 32.0f;
    //ウンコの初期化
    d->unko.img = d->unko.img;
    d->unko.px = 0.0f;
    d->unko.py = 0.0f;
    d->unko.vx = 0.0f;
    d->unko.vy = 5.0f;
...
void play(struct DATA* d) {
    //ウサギ移動
    if (isPress(KEY_D)) {
        d->rabit.px += d->rabit.vx;
    }
    else if (isPress(KEY_A)) {
        d->rabit.px -= d->rabit.vx;
    }
    //ウサギ移動制限
    if ((d->rabit.px + d->rabit.halfW) > width) {
        d->rabit.px = width - d->rabit.halfW;
    }
    if ((d->rabit.px - d->rabit.halfW) < 0) {
        d->rabit.px = 0 + d->rabit.halfW;
    }
    //ゴリラ移動
    d->gori.px += d->gori.vx;
    if (d->gori.halfW > d->gori.px ||
    ...
    if (d->ninjin.hp == 1) {
        ...
        if (d->ninjin.py < 0 - d->ninjin.halfW) {
            d->ninjin.hp = 0;
        }
    }
    //ウンコの状態書き換え
    if (d->unko.hp == 0) {
        d->unko.px = d->gori.px;
        d->unko.py = d->gori.bulletOfsY + d->gori.py;
        d->unko.hp = 1;
    }
    if (d->unko.hp == 1) {
        d->unko.py += d->unko.vy;
...
void draw(struct DATA* d){
    clear(255);
    rectMode(CENTER);
    image(d->backImg,width/2,height/2);
    //ウサギとゴリラ
...
void result(struct DATA* d) {
    ...
    draw(d);
    if (d->frameRestrictInput > 0) {
        d->frameRestrictInput--;
        d->ninjin.py += d->ninjin.vy;
        d->unko.py += d->unko.vy;
    }
    else {
      text("Spaceでタイトルに戻ります", 25, height);
      if (d->state == d->RESULT && isTrigger(KEY_SPACE)) {
        d->state = d->TITLE;
      }
    }
}
